	...
	stringBuilder.appendLine('<dokument>');
		...
		stringBuilder.appendLine('<telo>');
            ...
            stringBuilder.appendLine('\t<r49>');
                stringBuilder.appendLine('\t\t<rok>2016</rok>');
				stringBuilder.appendLine('\t\t<udaje>');
					...
					stringBuilder.appendLine('\t\t\t<r05>' + htmlEncode($('#R5_49').val()) + '</r05>');
				stringBuilder.appendLine('\t\t</udaje>');
			stringBuilder.appendLine('\t</r49>');
            ...
            stringBuilder.appendLine('\t\t<rok>2017</rok>');
                stringBuilder.appendLine('\t\t<udaje>');
                    ...
                    stringBuilder.appendLine('\t\t\t<r05>' + htmlEncode($('#R5_50').val()) + '</r05>');
                stringBuilder.appendLine('\t\t</udaje>');
			stringBuilder.appendLine('\t</r50>');
			stringBuilder.appendLine('\t<r51>');
                ...
                stringBuilder.appendLine('\t\t<udaje>');
                    ...
                    stringBuilder.appendLine('\t\t\t<r04>' + htmlEncode($('#R4_51').val()) + '</r04>');
                    stringBuilder.appendLine('\t\t\t<r05>' + htmlEncode($('#R5_51').val()) + '</r05>');
                stringBuilder.appendLine('\t\t</udaje>');
			stringBuilder.appendLine('\t</r51>');
			stringBuilder.appendLine('\t<r52>');
                stringBuilder.appendLine('\t\t<rok>2019</rok>');
                stringBuilder.appendLine('\t\t<udaje>');
			...
			stringBuilder.appendLine('\t\t</obchMeno>');
			stringBuilder.appendLine('\t\t<ulica>' + htmlEncode($('#tbUlica152').val()) + '</ulica>');
			stringBuilder.appendLine('\t\t<cislo>' + htmlEncode($('#tbCislo152').val()) + '</cislo>');
			stringBuilder.appendLine('\t\t<psc>' + htmlEncode($('#tbPSC152').val()) + '</psc>');
			stringBuilder.appendLine('\t\t<obec>' + htmlEncode($('#tbObec152').val()) + '</obec>');
...
function processTelo(telo) {
        ...
        switch(aNode.nodeName)
        {
		  ...
		  case 'r34':
                ...
                for (var j = 0; j < aNode.childNodes.length; j++) {
                    ...
                    if (bNode.nodeName == "dieta") {
                        ...
                        if (j != 0) { //&& j =< aNode.childNodes.length
                            var addButton = $('div[id^=layoutRow29731]').last().find('#addSectionlayoutRow29731');
		   ...
		   case 'r52': 
                ...
                $("#R4_52").val(getElementsValXml(udaje.getElementsByTagName('r04')[0]));
                $("#R5_52").val(getElementsValXml(udaje.getElementsByTagName('r05')[0]));
				break;
           ...
           case 'r152':
               for (var j = 0; j < aNode.childNodes.length; j++) {
                   ...
                   if (bNode.nodeName == "pravnaForma") $('#PravnaForma152').val(getNodeValue(bNode));
                   ...
                   if (bNode.nodeName == "ulica") $('#tbUlica152').val(getNodeValue(bNode));
                   if (bNode.nodeName == "cislo") $('#tbCislo152').val(getNodeValue(bNode));
                   if (bNode.nodeName == "psc") $('#tbPSC152').val(getNodeValue(bNode));
                   if (bNode.nodeName == "obec") $('#tbObec152').val(getNodeValue(bNode));
                    ...
                    if(l == 1)
                        ...
                        for (var k = 0; k < bNode.childNodes.length; k++) {
                            ...
                            if (cNode.nodeName == "kodStatu"){
								if(l>6)
								{
									var addButton = $('div[id^=layoutRow29632]').last().find('#addSectionlayoutRow29632');
	...
	if(prilPar30cOdpocetVydavkov.length != 0){
		for(var t = 0;t<prilPar30cOdpocetVydavkov.length;t++){
			...
			if(t != 0){
				addSection(1, 999, thisLR.find("[id^='addSectionlayoutRow29745']"));
				var policko = $("[id^='layoutRow29745']:last").find("[id^='Pri1r7']").attr("disabled",true);
...
function createNfdf() {
    ...
    var pocetvsetkychsekcii = $("[id^='layoutRow29745_']").length;
...
function createMainFdf() {
    ...
    stringBuilder.appendLine('    << /V (' + prepisznakyFdf($('#PravnaForma152').val()) + ')/T (152c)>>');
    var row = divideRows(2, 'tbObchMeno152');
    stringBuilder.appendLine('    << /V (' + prepisznakyFdf(row[0]) + ')/T (152d)>>');
    stringBuilder.appendLine('    << /V (' + prepisznakyFdf(row[1]) + ')/T (152e)>>');

    stringBuilder.appendLine('    << /V (' + prepisznakyFdf($('#tbUlica152').val()) + ')/T (152f)>>');
    stringBuilder.appendLine('    << /V (' + prepisznakyFdf($('#tbCislo152').val()) + ')/T (152g)>>');
    stringBuilder.appendLine('    << /V (' + prepisznakyFdf($('#tbPSC152').val()) + ')/T (152h)>>');
    stringBuilder.appendLine('    << /V (' + prepisznakyFdf($('#tbObec152').val()) + ')/T (152i)>>');
    ...
    var polesekcii1 = $("[id^='layoutRow29632']");
    ...
    var polesekcii2 = $("[id^='layoutRow29731']");
        ...
        for(var i = 4; i < polesekcii2.length; i++)
{
    ...
    stringBuilder.appendLine(' ]');
    stringBuilder.appendLine(' /F (form.495.DPFOB-20-print-save.pdf)');
...
function createpotvrdenie() {
    ...
    stringBuilder.appendLine(' ]');
    stringBuilder.appendLine(' /F (form.495.DPFOB-20-potvrdenie-print-save.pdf)');
...
function createopakovaneNfdf(davka, zaciatoksekcii, koniecsekcii, pocetsekciinastranu, pocetmaxstran) {
    ...
    var lr = $("[id^='layoutRow29745_']");
    var pocetFdfStran = lr.length + 1;
    var boolJePlne = false;

    if (pocetFdfStran == 1 && davka == 1) {
        var pomSekciaNaKOntrolu = $("[id='layoutRow29745_Repeating_1']");
    ...
    stringBuilder.appendLine('    ]');
    stringBuilder.appendLine('    /F (form.495.DPFOB-20-priloha10-print-save.pdf)');
