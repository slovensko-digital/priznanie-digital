...
function createxmlfinal() {
    ...
    var repeatingCount = $('[id^=layoutRow34277]').length;
    ...
    stringBuilder.appendLine('\t\t<rok>2017</rok>');
    ...
    stringBuilder.appendLine('\t\t\t<r05>' + htmlEncode($('#R5_48').val()) + '</r05>');
    stringBuilder.appendLine('\t\t</udaje>');
    stringBuilder.appendLine('\t</r48>');
   
    stringBuilder.appendLine('\t<r49>' + htmlEncode($('#t49').val()) + '</r49>');
    stringBuilder.appendLine('\t<r50>' + htmlEncode($('#t50').val()) + '</r50>');
    stringBuilder.appendLine('\t<r51>' + htmlEncode($('#t51').val()) + '</r51>');
    stringBuilder.appendLine('\t<r52>');
    stringBuilder.appendLine('\t\t<rok>2020</rok>');
    stringBuilder.appendLine('\t\t<udaje>');
    stringBuilder.appendLine('\t\t\t<r02>' + htmlEncode($('#R2_52').val()) + '</r02>');
    stringBuilder.appendLine('\t\t\t<r03>' + htmlEncode($('#R3_52').val()) + '</r03>');
    stringBuilder.appendLine('\t\t</udaje>');
    stringBuilder.appendLine('\t</r52>');
    ...
    for (var i = 0; i < 20; i++) {
        if (i == 0) {
            ...
            if (!document.getElementById("layoutRow34173_Repeating_" + i)) break;
            stringBuilder.appendLine('\t\t<udajeOprijmoch>');
...
function processTelo(telo) {
    for (var i = 0; i < telo.childNodes.length; i++) {
        ...
        switch (aNode.nodeName) {
            ...
            case 'r33':
                ...
                for (var j = 0; j < aNode.childNodes.length; j++) {
                    ...
                    if (bNode.nodeName == "dieta") {
                        ...
                        if (dietaCount > 4)
                            if (j != 0) {
                                var addButton = $('div[id^=layoutRow34277]').last().find('#addSectionlayoutRow34277');
            ...
            case 'r48':
                ...
                $("#R5_48").val(getElementsValXml(udaje.getElementsByTagName('r05')[0]));
                break;
          
            case 'r49': $('#t49').val(getNodeValue(aNode)); break;
            case 'r50': $('#t50').val(getNodeValue(aNode)); break;
                            ...
                            if (l == 1) {
                                ...
                                for (var k = 0; k < bNode.childNodes.length; k++) {
                                    ...
                                    if (cNode.nodeName == "kodStatu") {
                                        if (l > 6) {
                                            var addButton = $('div[id^=layoutRow34173]').last().find('#addSectionlayoutRow34173');
    ...
    if (prilPar30cOdpocetVydavkov.length != 0) {
        for (var t = 0; t < prilPar30cOdpocetVydavkov.length; t++) {
            var thisLR = $("[id^='layoutRow34291']:last");
            if (t != 0) {
                addSection(1, 999, thisLR.find("[id^='addSectionlayoutRow34291']"));
                var policko = $("[id^='layoutRow34291']:last").find("[id^='Pri1r7']").attr("disabled", true);
...
function createNfdf() {
    ...
    var pocetvsetkychsekcii = $("[id^='layoutRow34291_']").length;
...
function createMainFdf() {
    ...
    for (var i = 47; i <= 48; i++) {
        ...
        stringBuilder.appendLine('        [');
        stringBuilder.appendLine('          << /V (' + celacast($('#R2_' + i).val()) + ')/T (2)>>');
        stringBuilder.appendLine('          << /V (' + desatinnacast($('#R2_' + i).val(), 2) + ')/T (2d)>>');
        stringBuilder.appendLine('          << /V (' + celacast($('#R3_' + i).val()) + ')/T (3)>>');
        stringBuilder.appendLine('          << /V (' + desatinnacast($('#R3_' + i).val(), 2) + ')/T (3d)>>');
        stringBuilder.appendLine('          << /V (' + celacast($('#R4_' + i).val()) + ')/T (4)>>');
        stringBuilder.appendLine('          << /V (' + desatinnacast($('#R4_' + i).val(), 2) + ')/T (4d)>>');
        stringBuilder.appendLine('          << /V (' + celacast($('#R5_' + i).val()) + ')/T (5)>>');
        stringBuilder.appendLine('          << /V (' + desatinnacast($('#R5_' + i).val(), 2) + ')/T (5d)>>');
        stringBuilder.appendLine('    ]>>');
    }
    ...
    var polesekcii1 = $("[id^='layoutRow34173']");
    ...
    var polesekcii2 = $("[id^='layoutRow34277']");
    ...
    stringBuilder.appendLine(' ]');
    stringBuilder.appendLine(' /F (form.524.DPFOB-21-print-save.pdf)');
...
function createpotvrdenie() {
    ...
    stringBuilder.appendLine(' ]');
    stringBuilder.appendLine(' /F (form.524.DPFOB-21-potvrdenie-print-save.pdf)');
...
function createopakovaneNfdf(davka, zaciatoksekcii, koniecsekcii, pocetsekciinastranu, pocetmaxstran) {
    ...
    var lr = $("[id^='layoutRow34291_']");
    var pocetFdfStran = lr.length + 1;
    var boolJePlne = false;

    if (pocetFdfStran == 1 && davka == 1) {
        var pomSekciaNaKOntrolu = $("[id='layoutRow34291_Repeating_1']");
    ...
    stringBuilder.appendLine('    ]');
    stringBuilder.appendLine('    /F (form.524.DPFOB-21-priloha10-print-save.pdf)');
